...
\block{编码模型}
\trigger{当题目出现 \(\Xcal^L\)、码字集合 \(\mathcal{U}^k\)、编码映射 \(f\) 和译码映射 \(g\) 时，先画 \(X^L\to U^k\to\hat X^L\) 这条链。}

\begin{definition}[分组信源编码]
设离散无记忆信源的长度 \(L\) 输出序列取值于 \(\Xcal^L\)，码符号集合为
\(\mathcal{U}\)，每个码字长度为 \(k\)，则码字集合为 \(\mathcal{U}^k\)。编码映射和译码映射分别为
\[
f:\Xcal^L\to \mathcal{U}^k,
\qquad
g:\mathcal{U}^k\to \Xcal^L.
\]
整个过程可写为
\[
