...
以概率 \(\lambda\) 使用第一个信道、以概率 \(1-\lambda\) 使用第二个信道，
得到一个新的试验信道。其平均失真为
\[
...
新方案的失真和互信息分别是两个方案对应量的 \(\answerin[2cm]{\text{加权平均}}\)，
因此率失真函数的图像不会“向上弯”。
\workspace[2]
\end{yourturn}
...
对于离散信源，若失真测度满足
\[
d(x_i,\hat x_j)=0 \iff x_i=\hat x_j,
\]
则零失真时必须有 \(\hat X=X\)，因此
\[
...
所能达到的平均失真为 \(\answerin[2cm]{\text{最小}}\)。
\workspace[2]
\end{yourturn}
